...
export default function PlaceholdersAndVanishInput({
  ...
  // Draw text on canvas and extract pixel data
  ...
  // Animate the vanishing effect
  const animateFunc = (start) => {
  ...
  return (
    <form
      className={cn(
        "relative max-w-xl mx-auto bg-white h-12 rounded-full shadow transition duration-200 z-5000",
        value && "bg-gray-50"
      )}
    ...
    >
      ...
      <button
        disabled={!value}
        type="submit"
        className="absolute right-2 top-1/2 z-50 -translate-y-1/2 h-8 w-8 rounded-full disabled:bg-gray-100 bg-white transition duration-200 flex items-center justify-center"
      >
        <motion.svg
      ...
      <div className="absolute inset-0 flex items-center rounded-full pointer-events-none z-60">
